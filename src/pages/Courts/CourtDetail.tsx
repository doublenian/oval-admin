...
export const CourtDetail: React.FC = () => {
  ...
  const fetchCourtDetail = async () => {
    setLoading(true);
    try {
      const venueData = await venuesAPI.getVenue(courtId!);
      setVenue(venueData);
    } catch (error) {
      message.error('获取球场详情失败');
    } finally {
      setLoading(false);
    }
  };
          ...
          <Col>
            <Space>
              <Button
                ...
                onClick={() => navigate(`/courts/${venue.id}/images`)}
              >
                图片管理
              </Button>
              <Button
                icon={<EditOutlined />}
                onClick={() => navigate(`/courts/${venue.id}/edit`)}
              >
                编辑球场
              </Button>
              {venue.link && (
