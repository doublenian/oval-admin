...
export const IndoorBulkUpload: React.FC = () => {
  ...
  const downloadTemplate = () => {
    ...
    link.href = '/templates/indoor-court-template.xlsx';
    link.download = '室内球场信息导入模板.xlsx';
    link.click();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      message.success(`成功导入 ${uploadData.length} 条室内球场信息`);
      navigate('/indoor-courts');
    } catch (error) {
      message.error('导入失败');
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      <Card className="mb-4">
        <Title level={3}>室内球场信息批量上传</Title>
        <Paragraph type="secondary">
          通过Excel文件批量导入室内球场信息。如果球场已存在，将更新信息；如果不存在，将创建新球场。
        </Paragraph>
      </Card>
      ...
      <Card>
        ...
        {currentStep === 3 && (
          <div>
            ...
            <Alert
              message={`共解析到 ${uploadData.length} 条室内球场信息`}
              type="success"
              className="mb-4"
            />
